...
namespace SiGotvime__Web_.Helpers
{
    ...
    {
        public static string CreateImage(HttpPostedFileBase image)
        {
            ...
            string path = System.IO.Path.Combine(System.Web.HttpContext.Current.Server.MapPath("~/Content/images"), picTitle);
            int i = 0;
            while (System.IO.File.Exists(path))
            {

                picTitle = string.Format("{0}{1}.{2}", string.Concat(nameSplit), i, ext);
                path = System.IO.Path.Combine(System.Web.HttpContext.Current.Server.MapPath("~/Content/images"), picTitle);
                i++;
            }
            var img = Image.FromStream(image.InputStream, true, true);
            img.Save(path);
    ...
    public class ImageHelper
    {
        ...
        public ImageResult ScaleAndSave(Image image, string fileName)
        {
            var path = Path.Combine(HttpContext.Current.Request.MapPath(UploadPath), fileName);
            ...
            Image scaledImage = Scale(image);
            SaveTo(scaledImage, path, format, 80);
            return new ImageResult
            {
                Success = true,
                ImageName = fileName
