...
namespace Food_App_Service.Controllers
{
    ...
    public class RecipeController : ApiController
    {
        ...
        [HttpPost]
        public IEnumerable<SiGotvime.Data.DTO.RecipeDto> getThem([FromBody] int taken)
        {
            string baseUrl = Constants.rootUrl;
            var recipes = recipeRepository.GetLatest((taken / Constants.RecipesPerPage) + 1);

            List<SiGotvime.Data.DTO.RecipeDto> res = new List<SiGotvime.Data.DTO.RecipeDto>();
            foreach (var item in recipes)
            {
                SiGotvime.Data.DTO.RecipeDto r = recipeRepository.GetFullRecipeById(item);
                r.ID = item;
